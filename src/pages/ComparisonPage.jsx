...
function ComparisonPage() {
  ...
  const loadFromHistory = (item) => {
    setPolicyA({
      file: { name: item.nameA, size: item.sizeA || 0 },
      analysis: item.analysisA,
    });
    setPolicyB({
      file: { name: item.nameB, size: item.sizeB || 0 },
      analysis: item.analysisB,
    });
    setShowResults(true);
    ...
    try {
      const [resultA, resultB] = await Promise.all([
        analyzePolicy(
          policyA.file,
          (p, l) =>
            setProgress((prev) => ({ ...prev, a: p })) ||
            setProgressLabel((prev) => ({ ...prev, a: l })),
        ),
        analyzePolicy(
          policyB.file,
          (p, l) =>
            setProgress((prev) => ({ ...prev, b: p })) ||
            setProgressLabel((prev) => ({ ...prev, b: l })),
        ),
      ]);
      ...
      if (user?.uid) {
        ...
        setHistory((prev) => [
          { id, ...historyItem, displayDate: "Just now" },
          ...prev,
        ]);
      }
              ...
              <div>
                ...
                <h1 className="compare__title">
                  {showResults ? (
                    ...
                    <>
                      <span className="text-gradient--alt">Compare</span> two
                      policies
                    </>
                  )}
                </h1>
            ...
            <>
              <motion.div className="compare__slots" variants={itemVariants}>
                ...
                <input
                  ...
                  type="file"
                  accept=".pdf,.png,.jpg,.jpeg,.txt"
                  onChange={(e) => handleFile("b", e.target.files[0])}
                  hidden
                />
              </motion.div>
                      ...
                      >
                        <div className="compare__history-info">
                          <div className="compare__history-names">
                            <span className="compare__history-name">
                              {item.nameA}
                            </span>
                          ...
                          <p className="compare__history-date">
                            {item.displayDate}
                          </p>
                        </div>
                        <div className="compare__history-scores">
                          <div
                            className={`compare__history-score ${item.analysisA.coverageScore >= 80 ? "high" : item.analysisA.coverageScore >= 60 ? "mid" : "low"}`}
                          >
                            {item.analysisA.coverageScore}
                          </div>
                          <div
                            className={`compare__history-score ${item.analysisB.coverageScore >= 80 ? "high" : item.analysisB.coverageScore >= 60 ? "mid" : "low"}`}
                          >
              ...
              <div className="compare__scores">
                ...
                <div className="compare__score-vs">
                  <span className="compare__score-diff">
                    {Math.abs(
                      policyA.analysis.coverageScore -
                        policyB.analysis.coverageScore,
                    )}{" "}
                    pts
                  </span>
              ...
              <Card variant="lifted" className="compare__overview">
                ...
                <div className="compare__metrics">
                  {[
                    {
                      label: "Sum Insured",
                      a: policyA.analysis.policyOverview.sumInsured,
                      b: policyB.analysis.policyOverview.sumInsured,
                    },
                    {
                      label: "Premium",
                      a: policyA.analysis.policyOverview.premium,
                      b: policyB.analysis.policyOverview.premium,
                    },
                    {
                      label: "Provider",
                      a: policyA.analysis.policyOverview.provider,
                      b: policyB.analysis.policyOverview.provider,
                    },
                    {
                      label: "Validity",
                      a: `${policyA.analysis.policyOverview.effectiveDate} — ${policyA.analysis.policyOverview.expiryDate}`,
                      b: `${policyB.analysis.policyOverview.effectiveDate} — ${policyB.analysis.policyOverview.expiryDate}`,
                    },
                  ].map((metric) => (
                    <div key={metric.label} className="compare__metric-row">
                      <span className="compare__metric-val compare__metric-val--a">
                        {metric.a}
                      </span>
                      <span className="compare__metric-label">
                        {metric.label}
                      </span>
                      <span className="compare__metric-val compare__metric-val--b">
                        {metric.b}
                      </span>
                    </div>
                  ))}
                ...
                {activeTab === "coverage" && (
                  ...
                  >
                    <div className="compare__diff-header">
                      ...
                      <span className="compare__diff-col-label compare__diff-col-label--center">
                        Category
                      </span>
                    ...
                    {(() => {
                      const allCategories = [
                        ...new Set([
                          ...policyA.analysis.coverageItems.map(
                            (c) => c.category,
                          ),
                          ...policyB.analysis.coverageItems.map(
                            (c) => c.category,
                          ),
                        ]),
                      ];
                      return allCategories.map((cat) => {
                        const a = policyA.analysis.coverageItems.find(
                          (c) => c.category === cat,
                        );
                        const b = policyB.analysis.coverageItems.find(
                          (c) => c.category === cat,
                        );
                        ...
                        return (
                          <div
                            key={cat}
                            className={`compare__diff-row ${isDiff ? "compare__diff-row--highlight" : ""}`}
                          >
                            <span
                              className={`compare__diff-badge compare__diff-badge--${statusA}`}
                            >
                              {statusA === "covered"
                                ? "✓"
                                : statusA === "partial"
                            ...
                            <span className="compare__diff-category">
                              {cat}
                            </span>
                            <span
                              className={`compare__diff-badge compare__diff-badge--${statusB}`}
                            >
                              {statusB === "covered"
                                ? "✓"
                                : statusB === "partial"
                ...
                {activeTab === "exclusions" && (
                  ...
                  >
                    <div className="compare__exclusions-col">
                      <p className="compare__col-title">
                        Policy A — {policyA.analysis.exclusions.length}{" "}
                        exclusions
                      </p>
                      {policyA.analysis.exclusions.map((exc, i) => (
                        <div
                          key={i}
                          className={`compare__exclusion compare__exclusion--${exc.severity}`}
                        >
                          <span
                            className={`compare__severity compare__severity--${exc.severity}`}
                          >
                            {exc.severity}
                          </span>
                          <p className="compare__exclusion-title">
                            {exc.title}
                          </p>
                          <p className="compare__exclusion-detail">
                            {exc.detail}
                          </p>
                        </div>
                      ))}
                    ...
                    <div className="compare__exclusions-col">
                      <p className="compare__col-title">
                        Policy B — {policyB.analysis.exclusions.length}{" "}
                        exclusions
                      </p>
                ...
                {activeTab === "risks" && (
                  ...
                  >
                    <div className="compare__risks-col">
                      <p className="compare__col-title">
                        Policy A — {policyA.analysis.riskFlags.length} risks
                      </p>
                      {policyA.analysis.riskFlags.map((risk, i) => (
                        <Card
                          key={i}
                          variant="lifted"
                          className={`compare__risk-card compare__risk-card--${risk.level}`}
                          hoverable={true}
                        >
                          <span
                            className={`compare__risk-level compare__risk-level--${risk.level}`}
                          >
                            {risk.level === "critical"
                              ? "🔴"
                              : risk.level === "warning"
                    ...
                    <div className="compare__risks-col">
                      <p className="compare__col-title">
                        Policy B — {policyB.analysis.riskFlags.length} risks
                      </p>
        ...
        <div className="compare__slot-file">
          <div className="compare__slot-file-info">
            ...
            <div>
              ...
              <p className="compare__slot-filesize">
          ...
          {!comparing && (
            <button
              className="compare__slot-remove"
              onClick={(e) => {
                e.stopPropagation();
                onRemove();
              }}
          ...
          {comparing && (
            <div className="compare__slot-progress">
              <div className="compare__slot-progress-bar">
                <div
                  className="compare__slot-progress-fill"
